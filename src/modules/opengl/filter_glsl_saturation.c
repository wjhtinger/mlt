...
static const char *filter_glsl_saturation_frag=
...
"    vec3 mod = ( (col.rgb - luma) * saturation ) + luma;\n"
...
static int filter_get_image( mlt_frame frame, uint8_t **image, mlt_image_format *format, int *width, int *height, int writable )
{
	...
	*format = mlt_image_glsl;
	...
	glsl_shader shader = g->get_shader( g, "filter_glsl_saturation_frag", &filter_glsl_saturation_frag );
	if ( !shader ) {
		g->release_fbo( fbo );
	...
	glUniform1f( glGetUniformLocationARB( shader->program, "saturation" ), saturation );
	...
	if ( dest ) {
		fprintf(stderr,"filter_glsl_saturation -----------------------set_image, %u, %u [frame:%d] [%d] saturation=%f\n",
				(unsigned int)dest, dest->texture, mlt_properties_get_int(MLT_FRAME_PROPERTIES( frame ), "_position"), syscall(SYS_gettid), saturation);
...
{
	double saturation = mlt_properties_get_double( MLT_FILTER_PROPERTIES( filter ), "saturation" );
	saturation = (saturation < 0 || saturation > 10) ? 1 : saturation;
	mlt_properties_set_double( MLT_FRAME_PROPERTIES( frame ), "saturation", saturation );
...
mlt_filter filter_glsl_saturation_init( mlt_profile profile, mlt_service_type type, const char *id, char *arg )
{
	...
	if ( this )
	{
		...
		mlt_properties_set( MLT_FILTER_PROPERTIES( this ), "saturation", arg == NULL ? "1" : arg );
	}
	return this;
}
