...
static void _do_render(mlt_consumer consumer, mlt_frame frame);
...
static void _do_render(mlt_consumer consumer, mlt_frame frame)
{
	...
	mlt_log_error( MLT_CONSUMER_SERVICE(consumer), "_do_render11 data[%d][%d]!", width, height);
	mlt_frame_get_image(frame, &image, &vfmt, &width, &height, 0);
	if (image == NULL )
	{
		mlt_log_error( MLT_CONSUMER_SERVICE(consumer), "_do_render data[%d][%d]!", width, height);
		return;
	}
	mlt_log_error( MLT_CONSUMER_SERVICE(consumer), "_do_render22 data[%d][%d]!", width, height);
